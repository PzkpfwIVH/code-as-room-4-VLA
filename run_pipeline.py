...
def _load_dotenv(dotenv_path: Path) -> None:
    ...
    Does NOT override existing environment variables (standard dotenv
    behaviour). Supports:
      - KEY=VALUE
      - KEY="VALUE" / KEY='VALUE'
...
# Load .env from project root BEFORE any agent_utils imports, because
# several modules (stage3/core.py, stage11_texture.py, etc.) read
...
def _flatten_message_content(content: Any) -> str:
    ...
    if isinstance(content, dict):
        ...
        return _flatten_message_content(text)
...
def _flatten_messages_to_text(messages: List[Any]) -> List[Dict[str, Any]]:
    flattened = []
    for message in messages:
        if isinstance(message, dict):
            ...
            content = _flatten_message_content(getattr(message, "content", message))
...
def _patch_chatopenai_invoke() -> None:
    """Monkey-patch ChatOpenAI.invoke so that models without vision support
    automatically fall back to a dedicated vision model (when configured).

    Environment variables (read BEFORE this patch runs via _load_dotenv):
      SCENEGEN_VISION_MODEL   – model name for vision-capable LLM
      SCENEGEN_VISION_BASE_URL – API base URL for the vision model
      SCENEGEN_VISION_API_KEY  – API key for the vision model

    Fallback chain (first match wins):
      1. Try the original model with the original messages.
      2. If the error is ``unknown variant … image_url …`` AND a vision
         model is configured, retry with the vision model.
      3. If no vision model is configured, strip image content and
         retry with the original model (text-only → often produces
         empty / unusable results — a prominent warning is printed).
    """
    try:
    ...
    if getattr(ChatOpenAI.invoke, "__patched_for_image_fallback__", False):
        return

    # ── Read vision-model config from environment (already populated by
    #    _load_dotenv) ──────────────────────────────────────────────────
    _VISION_MODEL = os.environ.get("SCENEGEN_VISION_MODEL")
    _VISION_BASE_URL = os.environ.get("SCENEGEN_VISION_BASE_URL")
    _VISION_API_KEY = os.environ.get("SCENEGEN_VISION_API_KEY") or os.environ.get("SCENEGEN_API_KEY") or os.environ.get("OPENAI_API_KEY")

    # Lazy-constructed vision client (created once, reused for all fallbacks)
    _vision_client: Any = None

    def _get_vision_client() -> Any:
        nonlocal _vision_client
    ...
    def patched_invoke(self, messages, *args, **kwargs):
        try:
            ...
            err_str = str(exc)
            if "unknown variant" in err_str and "image_url" in err_str:
                # ── Step 2: try vision model ──────────────────────────
                if _VISION_MODEL:
                    try:
                        model_name = getattr(self, "model_name", "unknown")
                        print(
                            f"⚠️  Model '{model_name}' rejected image content. "
                            f"Retrying with vision model: {_VISION_MODEL} "
                            f"@ {_VISION_BASE_URL or '(default)'}"
                        )
                        return _get_vision_client().invoke(messages, *args, **kwargs)
                    except Exception as vision_exc:
                        print(
                            f"❌ Vision-model fallback also failed: {vision_exc}\n"
                            f"   Falling through to text-only fallback (results may be empty)."
                        )
                # ── Step 3: strip images (last resort) ────────────────
                fallback_messages = _flatten_messages_to_text(messages)
                print(
                    "⚠️  NO VISION MODEL CONFIGURED — stripping image content!\n"
                    "   The LLM cannot see the reference image and WILL return "
                    "empty/useless results.\n"
                    "   Set SCENEGEN_VISION_MODEL / SCENEGEN_VISION_BASE_URL / "
                    "SCENEGEN_VISION_API_KEY in your .env file."
                )
                return original_invoke(self, fallback_messages, *args, **kwargs)
            raise

    patched_invoke.__patched_for_image_fallback__ = True
...
# ── Apply ChatOpenAI vision fallback patch ────────────────────────────
# Must run AFTER stage imports (which pull in langchain_openai).
_patch_chatopenai_invoke()
...
class PipelineConfig:
    ...
    def __init__(
        ...
        blender_path: str = "D:\\Blender 4.5.3\\blender.exe",
        ...
        self.stage8_small_describe_model = None

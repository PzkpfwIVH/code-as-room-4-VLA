...
class Stage3RotationRunner:
    ...
    def _render_topdown(self, iteration: int) -> Optional[str]:
        ...
        # Escape backslashes for Windows paths injected into Python code.
        code_file_esc  = code_file.replace("\\", "\\\\")
        render_image_esc = render_image.replace("\\", "\\\\")
        render_script = self._build_render_script(code_file_esc, render_image_esc)
...
Output JSON only."""},
        ...
        try:
            ...
            with open(os.path.join(self.output_dir, f"_rot_analysis{suffix}.txt"), "w", encoding="utf-8") as f:
                f.write(raw)
...
code_text = open("{code_file}", "r", encoding="utf-8").read()

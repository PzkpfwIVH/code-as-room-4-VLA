...
class Stage3Pipeline:
    ...
    def _render_scene(self) -> Optional[str]:
        ...
        try:
            ...
            # Escape backslashes for Windows paths: `\r`, `\n`, `\t`, etc.
            # in a Windows path like "...\run_xxx\..." would otherwise be
            # interpreted as escape sequences when exec()'d inside Blender.
            code_file_escaped = code_file.replace("\\", "\\\\")
            output_image_escaped = output_image.replace("\\", "\\\\")
            ...
            with open(render_file, "w", encoding="utf-8") as f:
                f.write(render_script.format(
                    code_file=code_file_escaped,
                    output_image=output_image_escaped
                ))
...
import os
    ...
    # Execute the layout code (read with explicit UTF-8 to avoid locale issues)
    exec(open("{code_file}", "r", encoding="utf-8").read())

...
"""
    ...
    def _render_scene(self) -> str | dict:
        ...
        # Escape backslashes for Windows: \r, \n, \t, etc. in paths like
        # "...\run_xxx\..." would be interpreted as escape sequences when
        # Blender exec()'s the generated render script.
        code_file_esc  = code_file.replace("\\", "\\\\")
        render_image_esc = render_image.replace("\\", "\\\\")
        layout_json_esc  = layout_json.replace("\\", "\\\\")
        render_script = f'''
import bpy
import sys
import math
...
code_text = open("{code_file_esc}", "r", encoding="utf-8").read()
...
with open("{layout_json_esc}", "w", encoding="utf-8") as _f:
...
bpy.context.scene.render.filepath = "{render_image_esc}"
...
Output the COMPLETE fixed Python script. Do NOT omit any part of the code."""
    ...
    def run(self, iterate: bool = True) -> tuple:
        ...
        # Save initial code
        os.makedirs(self.output_dir, exist_ok=True)
        with open(os.path.join(self.output_dir, "stage3_initial.py"), "w", encoding="utf-8") as f:
            f.write(self.current_code)
            ...
            for i in range(self.max_iterations):
                ...
                if isinstance(rendered, dict) and "error" in rendered:
                    ...
                    if self._fix_code_errors(rendered["error"]):
                        with open(os.path.join(self.output_dir, f"stage3_iter{i+1}.py"), "w", encoding="utf-8") as f:
                            f.write(self.current_code)
                    continue
                if not rendered:

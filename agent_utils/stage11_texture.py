...
# =============================================================================
# DashScope (Aliyun) text-to-image adapter
# =============================================================================

def _is_dashscope_endpoint(base_url: str) -> bool:
    """Return True if base_url points to a DashScope / Aliyun endpoint."""
    if not base_url:
        return False
    url_lower = base_url.lower()
    return any(kw in url_lower for kw in ("dashscope", "aliyuncs.com"))


def _is_wan_image_model(model: str) -> bool:
    return bool(model and model.lower().startswith("wan2."))


def _is_qwen_image_model(model: str) -> bool:
    """Return True if *model* is a Qwen image-generation model that uses
    the DashScope multimodal-generation endpoint."""
    if not model:
        return False
    m = model.lower()
    return m.startswith("qwen-image") or m in (
        "qwen-image-plus", "qwen-image-max", "qwen-image-flash",
    )


def _is_multimodal_image_model(model: str) -> bool:
    """Models that use DashScope multimodal-generation endpoint."""
    return _is_wan_image_model(model) or _is_qwen_image_model(model)


def _parse_dashscope_image_url_from_response(result: dict) -> str:
    # Wan2.x models return messages content with image items in the assistant reply.
    output = result.get("output", {})
    choices = output.get("choices", [])
    if not choices:
        raise ValueError(f"No image choices in DashScope response. Keys: {list(result.keys())}")

    message = choices[0].get("message", {})
    content = message.get("content", [])
    if not isinstance(content, list):
        raise ValueError(f"Unexpected DashScope choice content format: {type(content)}")
    ...
    for item in content:
        ...
        if isinstance(item.get("url"), str):
            return item["url"]
    raise ValueError(
        f"No image URL found in DashScope response content. "
        f"Choice content keys: {[list(item.keys()) for item in content if isinstance(item, dict)]}"
    )
...
def _call_dashscope_image(
    ...
    """Call DashScope image generation API for Wan models and text2image models.

    Wan2.x image models use the new DashScope multimodal-generation sync endpoint:
        POST {host}/api/v1/services/aigc/multimodal-generation/generation

    Older DashScope text2image models use the legacy text2image endpoint:
        POST {host}/api/v1/services/aigc/text2image/image-synthesis
    """
    import requests
    from urllib.parse import urlparse

    # Map our size spec to DashScope's size format.
    # DashScope accepts e.g. "1024*1024", "720*1280", "1280*720", "2048*2048".
    _SIZE_MAP = {
        "1K": 1024,
        "2K": 2048,
        "4K": 4096,
    }
    px = _SIZE_MAP.get(str(image_size).upper(), 1024)
    size_str = f"{px}*{px}"

    parsed = urlparse(base_url or "https://dashscope.aliyuncs.com")
    ...
    if _is_multimodal_image_model(model):
        ...
        # wan2.x models require prompt_extend=False; qwen-image models
        # don't support the key, so only add it for wan models.
        if _is_wan_image_model(model):
            payload["parameters"]["prompt_extend"] = False
    else:
        url = f"{host}/api/v1/services/aigc/text2image/image-synthesis"
        payload = {
            "model": model,
            "input": {
                "prompt": prompt_text,
    ...
    image_url = None
    if _is_multimodal_image_model(model):
        image_url = _parse_dashscope_image_url_from_response(result)
    else:
        results = result.get("output", {}).get("results", [])
        if not results:
            raise ValueError(
                f"No image in DashScope response. Keys: {list(result.keys())}. "
                f"Snippet: {json.dumps(result, ensure_ascii=False)[:1500]}"
            )
        image_url = results[0].get("url")
        if not image_url:
            raise ValueError(f"DashScope result missing 'url': {results[0]}")

    # Download the actual image bytes from the temporary URL
    img_resp = requests.get(image_url, timeout=timeout)
    if not img_resp.ok:
        raise RuntimeError(
            f"Failed to download image from DashScope URL: "
...
def _call_openai_image_api(
    ...
    Used for DashScope compatible-mode or any other provider whose image
    API follows the OpenAI DALL·E wire format.

    Returns raw image bytes (PNG or WebP, provider-dependent).
    """
    import requests

    _SIZE_MAP = {"1K": "1024x1024", "2K": "2048x2048", "4K": "4096x4096"}
    size_str = _SIZE_MAP.get(str(image_size).upper(), "1024x1024")

    root = base_url.rstrip("/")
    # Handle the case where base_url already ends with /v1
    if root.endswith("/v1"):
    ...
    resp = requests.post(url, json=payload, headers=headers, timeout=timeout)
    if not resp.ok:
        # DashScope compatible-mode may reject synchronous calls with 403.
        # When we detect that, automatically retry with X-DashScope-Async.
        if resp.status_code == 403 and "synchronous" in resp.text.lower():
            return _call_openai_image_api_async(
                base_url=base_url,
                api_key=api_key,
                model=model,
    ...
    if not data:
        raise ValueError(f"No 'data' in response. Keys: {list(result.keys())}")
    ...
    raise ValueError(f"No image data in response. Item keys: {list(data[0].keys())}")


def _call_openai_image_api_async(
    base_url: str,
    api_key: str,
    model: str,
    prompt_text: str,
    ...
    """Call OpenAI-compatible /v1/images/generations with X-DashScope-Async: enable.

    DashScope compatible-mode async image generation flow:
      1. POST /v1/images/generations  (header: X-DashScope-Async: enable)
      2. Receive {"task_id": "...", "status": "PENDING"}
      3. Poll GET /v1/tasks/{task_id} until SUCCEEDED / FAILED
      4. Extract b64_json or download URL from the final response.

    Returns raw image bytes (PNG / WebP, provider-dependent).
    """
    import requests
    import time
    import base64
    ...
    root = base_url.rstrip("/")
    if root.endswith("/v1"):
        images_url = f"{root}/images/generations"
        tasks_base = root
    else:
        images_url = f"{root}/v1/images/generations"
        tasks_base = f"{root}/v1"
    ...
    # 1. Submit async task
    resp = requests.post(images_url, json=payload, headers=headers, timeout=timeout)
    if not resp.ok:
        raise RuntimeError(
            f"Async submit HTTP {resp.status_code}: {resp.text[:2000]}"
        )
    ...
    try:
        ...
        raise ValueError(
            f"Invalid JSON in async submit response: {e}\n{resp.text[:2000]}"
        ) from e

    task_id = result.get("task_id")
    if not task_id:
        # Might have completed synchronously despite the header — try
        # parsing inline image data immediately.
        data = result.get("data", [])
        if data:
            b64 = data[0].get("b64_json")
            if b64:
            ...
            if img_url:
                ...
                return img_resp.content
        raise ValueError(
            f"No task_id in async response. Keys: {list(result.keys())}"
        )

    # 2. Poll until terminal state
    task_url = f"{tasks_base}/tasks/{task_id}"
    ...
    while time.time() < deadline:
        ...
        if not poll_resp.ok:
            raise RuntimeError(
                f"Task poll HTTP {poll_resp.status_code}: {poll_resp.text[:2000]}"
            )

        task_result = poll_resp.json()
        status = (task_result.get("status") or "").upper()

        if status == "SUCCEEDED":
            # Try the standard OpenAI data path first
            data = task_result.get("data", [])
            if not data:
                # DashScope sometimes nests under output.{data,results}
                output = task_result.get("output", {}) or {}
                data = output.get("data", []) or output.get("results", [])
            ...
            raise ValueError(
                f"Async task {task_id} SUCCEEDED but no image data found. "
                f"Keys: {list(task_result.keys())}"
            )

        if status in ("FAILED", "ERROR", "CANCELLED"):
            error_msg = task_result.get(
                "message", task_result.get("error", "unknown failure")
            )
            raise RuntimeError(f"Async task {task_id} {status}: {error_msg}")

        # Still PENDING / RUNNING → continue polling

    raise TimeoutError(
        f"Async task {task_id} did not reach terminal state within {max_wait}s"
    )
...
def _generate_texture_image(
    ...
    """Unified dispatcher: route to the correct image generation API.

    Detection priority:
      1. DashScope native (aliyuncs.com / dashscope) → DashScope text2image API
      2. Try OpenAI-compatible /v1/images/generations first (for DashScope
         compatible-mode etc.)
      3. Fall back to Gemini generateContent API
    """
    if _is_dashscope_endpoint(base_url):
        # Check for OpenAI-compatible mode pattern.
        # qwen-image models do NOT work through the compatible-mode
        # /v1/images/generations endpoint — skip straight to native API.
        if "/compatible-mode" in base_url.lower() and not _is_qwen_image_model(model):
            # DashScope compatible-mode: try OpenAI images format first
            try:
                ...
                # Fall through to native DashScope API
                pass
        # DashScope native text2image / multimodal-generation
        return _call_dashscope_image(
            base_url=base_url,
            api_key=api_key,
            model=model,
            prompt_text=prompt_text,
    ...
    # Default: Gemini generateContent API (existing behaviour)
    if not GEMINI_IMAGE_AVAILABLE:
        raise RuntimeError(
            "Gemini image generation is not available "
            "(topdown_room_image_generator._call_gemini_image not found)"
        )
...
class StageTextureRunner:
    ...
    def _generate_one(
        ...
        *,
        ...
        for attempt in range(1, retries + 1):
            try:
                img_bytes = _generate_texture_image(
    ...
    def _generate_all_textures(self) -> Dict[str, str]:
        ...
        if not GEMINI_IMAGE_AVAILABLE and not _is_dashscope_endpoint(self.texture_base_url or ""):
            self._log("Image generation unavailable (no Gemini client and not a DashScope endpoint)", "error")
            return results
    ...
    def _save_outputs(self, code: str, results: Dict[str, str]) -> str:
        ...
        try:
            from blender_code_syntax_fix import (
                ...
                fix_unlink_collection_arg,
            )
            code = fix_empty_if_show_direction_before_return(code)
            code = fix_unlink_collection_arg(code)
        except Exception as e:
